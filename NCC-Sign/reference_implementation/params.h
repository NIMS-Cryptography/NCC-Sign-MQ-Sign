...
#else
#define POLYZ_PACKEDBYTES ((N * 19) / 8)	// 5prime 포함
#endif

#if (N == 2304)
#define POLYW1_PACKEDBYTES ((N * 4) / 8)
#else
#define POLYW1_PACKEDBYTES ((N * 5) / 8)	// 5prime 포함
#endif

#define POLYETA_PACKEDBYTES ((N * 3) / 8)

#define CRYPTO_PUBLICKEYBYTES (SEEDBYTES + POLYT1_PACKEDBYTES)
#define CRYPTO_SECRETKEYBYTES (3 * SEEDBYTES + 2 * POLYETA_PACKEDBYTES + POLYT0_PACKEDBYTES)
